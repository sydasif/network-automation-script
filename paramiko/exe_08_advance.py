...
def issue_command(channel, command, delay=1):
    connection = channel
    command_str = command + "\n"
    connection.send(command_str.encode())
    time.sleep(delay)
    output = connection.recv(99999)
    return output


# Sets up the ssh session and log in as "admin" with password "cisco" to host
# '192.168.100.22'. Also added "look_for_keys=False" and "allow_agent=False".
try:
...
if connection:
    ...
    print(f"Sorry, there is no connection to the host {host}")
